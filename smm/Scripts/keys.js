...
//            if(event.keyCode=='P'.charCodeAt())if(marioctrl==true){pause();socket.emit("pause",start)}
    ...
    else{           if(marioctrl==true && elsebutton=="fireball") {keys.accelerate = status;socket.emit("mario1",salon+"&"+status)}
                    if(marioctrl==true && elsebutton=="down") {keys.down = status;socket.emit("mario2",salon+"&"+status)}
                    if(marioctrl==true && elsebutton=="right") {keys.right = status;socket.emit("mario3",salon+"&"+status)}
                    if(marioctrl==true && elsebutton=="left") {keys.left = status;socket.emit("mario4",salon+"&"+status)}
                    if(marioctrl==true && elsebutton=="up") {keys.up = status;socket.emit("mario5",salon+"&"+status)}
				return true
